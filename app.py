...
if not data_db:
    ...
    if search_brg:
        df_download = df_download[df_download["Nama Barang"].str.contains(search_brg, na=False) | df_download["Kode Barang"].str.contains(search_brg, na=False)]
    ...
    if not df_download.empty:
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            df_excel_final.to_excel(writer, index=False, sheet_name='Laporan Stok Akhir')
            
        st.download_button(label="📥 Download Laporan ke Excel", data=buffer.getvalue(), file_name=f"Laporan_Stok_{datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
